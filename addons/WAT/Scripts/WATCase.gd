...
func _add_expectation(success: bool, expectation: String, got: String, notes: String) -> void:
	# Called via signal from expectations.gd
	self._tests[self._current_method].expectations.append({"details": expectation, "success": success, "got": got, "notes": notes})
	if not success:

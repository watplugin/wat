...
#	Output.connect("finished", Runner, "_finish")
	...
	Runner.connect("display_results", Results, "_display_results", [], CONNECT_DEFERRED)
#	Runner.connect("ended", Results, "_display_results")
	Runner.connect("ended", TotalTimer, "_stop")
#	Runner.connect("clear", Output, "_clear")
#	Runner.connect("clear", Results, "_clear")
	Runner.connect("started", Output, "_clear")
